...
class ChatApp(App):
    ...
    async def _handle_command(self, text: str) -> None:
        ...
        if cmd == "/dm":
            ...
            self._welcome_cmd("/update", "check for and apply updates", width=21)
            self._welcome_cmd("/restart", "restart rootchat", width=21)
            self._welcome_cmd("/welcome", "toggle startup welcome message", width=21)
            self._welcome_cmd("/rooms", "list active rooms on the relay", width=21)
            self._welcome_cmd("/room <name>", "switch relay room", width=21)
            self._welcome_cmd("/name <newname>", "change your username", width=21)
            self._welcome_cmd("/dm <user>", "open a private chat", width=21)
            self._welcome_cmd("/msg <user> <text>", "send a one-off private message", width=21)
            self._welcome_cmd("/connect", "list saved connections", width=21)
            self._welcome_cmd("/connect <n>", "switch to connection n", width=21)
            self._welcome_cmd("/connect new", "add a new connection", width=21)
            self._welcome_cmd("/connect edit <n>", "edit connection n", width=21)
            self._welcome_cmd("/connect delete <n>", "delete connection n", width=21)
            self._welcome_cmd("/add <username>", "add to contacts", width=21)
            self._welcome_cmd("/remove <username>", "remove from contacts", width=21)
            self._welcome_cmd("/autostart", "toggle launch on system login", width=21)
            self._welcome_cmd("/mute", "mute notifications", width=21)
            self._welcome_cmd("/unmute", "re-enable notifications", width=21)
            self._welcome_cmd("/quit", "exit", width=21)
        else:
